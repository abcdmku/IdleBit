...
| CRON v1 timer automation | Tested | CRON Scheduler unlocks timer repeats for visible repeatable system tasks only; default minimum 60s, seconds/minutes modes, whole-second next-job countdown, increasingly expensive `cronInterval` upgrades lower the minimum by 1 second each, skipped runs do not catch up |

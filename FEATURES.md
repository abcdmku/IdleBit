...
| CRON v1 timer automation | Tested | CRON Scheduler unlocks timer repeats for visible repeatable system tasks only; default minimum 60s, seconds/minutes modes, whole-second next-job countdown, `cronInterval` lowers the minimum by 1 second per upgrade, skipped runs do not catch up |
...
| Second CPU unlock | Tested | Multi-core benchmark compute enables System Bus research; the CPU socket offers unmatched and matched CPU installs, each listing its projected power increase, and purchasing either reveals CRON Scheduler research while PSU stays visible from the start |
...
| CRON module reveal | Tested | Second CPU purchase reveals CRON Scheduler research only; the CRON system module stays hidden until that research is bought |
...
| RAM and PSU readouts | Tested | RAM appears above the CPU, shows per-stick Stage/Load/Ready lanes in a selectable cache-style stick array with an All target, reports module frequency without summing stick speeds, and exposes new-stick plus stick capacity/frequency upgrade access; PSU readouts target draw, billing, state, capacity, load, wattage upgrades, and overload failure pressure from the first screen |
...
- Verification completed May 19, 2026: `npm test`, `npm run typecheck`, `npm run build`, and `git diff --check` passed for unmatched/matched CPU socket choices with power-delta readouts, credits-only first-screen PSU Capacity, compact PSU readouts, compact header power controls, larger centered PSU overload header progress, full-card red PSU over-power flashing, first-time short PSU failure popup with repeat-failure topbar badge, removed headroom/efficiency row, and the 10-second PSU overload failure pressure.

...
| Progressive task reveal | Tested | Fetch Bit and Decode Bit are the intentional fresh-save pair; every other player job stays absent until all of its prerequisite research is complete, while unmet hardware or prior-job requirements may leave researched work visibly blocked. Research benchmarks appear only on their owning research cards, and the task panel groups visible work by CPU-bound, system, distributed, and other categories. |
...
| Multi-core unlock | Tested | Micro Benchmark enables Multi-Core Control, buying the second core reveals the two-core Parallelism Benchmark, and that benchmark completes the Local Scheduler research gate; small eligible multicore work can use the installed cores directly without requiring scheduler slots |

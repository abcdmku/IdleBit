...
| Cache upgrades | Tested | Capacity and speed upgrades are available from the start; capacity costs more data than credits, speed follows the CPU tier frequency ladder with target-level costs multiplied by installed cores, and both improve cache queue/fill behavior |
...
| Second CPU unlock | Tested | Multi-core benchmark compute enables System Bus research; CPU install paths add a level-1 one-core package matching the system's existing CPU tier with projected power increase, and purchasing a CPU reveals CRON Scheduler research while PSU stays visible from the start |
| CPU package choices | Tested | Existing-system CPU purchases keep the system's installed CPU tier; copied-package CPU install semantics are hidden, so new packages no longer copy another package's level, cores, cache, cache speed, or scheduler slots |
...
| Browser persistence | Tested | Save/load works in browser storage, browser persistence writes `save-v4`, and incompatible v3-or-older saves reset to a clean v4 initial state for the RAM block/channel model |
...
| Component-scoped controls | Tested | CPU, CPU-local cache, CPU-local scheduler, RAM, socket, and PSU expose relevant local actions and readouts; cheap credits-only PSU wattage upgrades are available from the start while advanced PSU tuning and Thermal controls are deferred |
...
| CPU package tuning controls | Tested | CPU Package Level is purchased from the selected CPU package control strip; after CPU Operation Scheduler unlock, a Cores-header All selector targets the same package-level +/- control without becoming a task route, and Add Core stays beside it instead of repeating controls in every core tile |

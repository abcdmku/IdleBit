...
| Automation Buffer | Tested | Nine sequential levels (the starting node plus eight purchases) progress from 0 to 168 hours; the first card explains that it runs only already-queued work while closed, shows the exact coverage increase, and reveals an installed-status view immediately after purchase; each late maximum has a separate chapter research gate, departure-owned capacity caps catch-up, and overflow is harmless and non-retroactive |
...
| Bootstrap/Coherent campaign | Tested | Data-driven opening objectives teach queue, buffer, first benchmark, multicore, RAM, schedulers, second CPU, and CRON with predictable per-completion task payouts |
...
| Contracts and projects | In Progress | Deterministic sustained/burst offers enforce a 15-minute refresh cadence, three-contract capacity, active-template novelty reservation, and compact exact rate readouts; contract template eligibility and `refreshContractMarket` are CRON-gated with a public blocker and stale offers are pruned at load; productive progress follows frozen ordered cache/RAM/CPU and later storage/network work on the assigned system, incompatible target systems are rejected with explicit lane blockers instead of fake maximum ETAs, every Credit settles from exact recipe work plus a named frozen multiplier, offer expiry remains a deadline, and phased optional arcs persist exact stage progress without blocking the mainline |
...
| Command-deck UI | Tested | Compact desktop topbar and mobile Jobs omit campaign stages/objective instructions, while exceptional persistence status, contextual Automation Buffer controls in Work, Work taxonomy, projections, return summary, named Fleet, preset/Advanced builder, responsive mobile-first Work, zoom/reflow, touch targets, keyboard/focus, reduced motion, globally non-selectable interface text, and zero-violation tagged WCAG Axe coverage pass the browser acceptance suite |
| Progressive hardware-owned work UI | In Progress | Opening Work exposes only playable Jobs; Automation reveals after the first buffer purchase or with System Scheduler/CRON, Campaign reveals with System Scheduler research (legacy saves with an active/completed project keep Campaign), Contract Market reveals with CRON Scheduler research (or while legacy contracts exist), Standing Orders join Automation at CRON, and Automation Buffer levels stay R&D-column purchase cards while Automation shows installed coverage, capability, finite-queue limits, and departure forecast; campaign stage names and objective instructions never render in the topbar or Jobs view; task cards avoid redundant projections; fixed-height system status shows active projects/contracts/orders/jobs without shifting surrounding UI |
...
| Opening physical timing | Tested | Fetch Bit is exactly 2 ops and two seconds at starter 1 Hz rates; Decode Bit is 2 ops with a 2 b cache footprint and exactly 4 paid hardware-work units, yielding a 4-Credit gross payout; one transfer bit or CPU op takes one second on its applicable 1 Hz lane, and Bit Flip is exactly 3 ops, so it is strictly slower on identical hardware |
...
| Smooth exact progress meters | Tested | Foreground state, including cache activity, targets publication every 10 ms (frame-aligned by the browser); Work, project, contract, cluster, Cloud, storage, Live Ops, core, cache, RAM, rack, and queue fills use 9 ms interpolation between exact snapshots, snap batch resets, and disable motion under reduced-motion preference |
...
| Manual tasks | Tested | Every completion pays work-derived Credits plus one whole Data per ten gross Credits, rounded down with a one-Data minimum for small public tasks; Fetch Bit is runnable without a Data purchase and pays 1 Data every completion, so the opening never requires spending Data before Data can be earned |
| Work-derived payouts | Tested | One exact paid-work ledger covers Jobs, paid benchmarks, every project phase, contracts, Live Operations, Workshop storage, cluster work, and Cloud SLAs; sequential lane work pays one Credit per unit before an explicit named frozen multiplier, public task Data derives at a 1:10 ratio from that gross Credit payout, concurrent memory issue/cache transfer is counted once, hardware speed changes duration rather than gross payout, and closed-world balance tests reject untracked Credit sources |
| Data-storage capacity pricing | Tested | Cache capacity, RAM installs and size upgrades, persistent storage, server memory, and accelerator device memory use one exact 10 Data per 1 Credit cost rule; speed and non-data limits such as PSU, thermal, queue, network, offline-time, and facility capacity remain independently priced |
...
| Task operation composition | Tested | CPU-bound tasks decompose into read/write/overwrite memory operations and compute operations; every player-facing task with RAM staging is owned by the System Scheduler and remains hidden until that research is complete, while composed system/distributed stages preserve child provenance; cards report CPU cycle work once as `ops`, while DAG chips keep cache and RAM transfer bits separate |
| Runtime progress meters | Tested | Task cards show one whole-task progress value across recipe/load/compute work, RAM shows reserved/loading/ready staging progress, and CPU cores use the same concise track, fill, and motion treatment as the PSU Draw meter for current execution, including memory-operation cache issue cycles |
...
| Equal-rate cache display | Tested | When one core and its cache run at the same rate, issued bits move directly into Ready with no Buffer state or hidden striped Buffer layer; Buffer appears only for meaningful positive backpressure when aggregate CPU issue exceeds cache bandwidth |
| Progressive task reveal | Tested | Fetch Bit and Decode Bit are the intentional fresh-save pair; normal jobs stay absent until their reveal-gating research is complete. Installing core #2 deliberately previews Parallel Bit Count and Dual-Stream Decode as blocked two-core CPU-bound work so Local Scheduler and its two-open-slot requirement are discoverable. Research benchmarks appear only on their owning research cards, and the task panel groups visible work by CPU-bound, system, distributed, and other categories. |
| Progressive research reveal | Tested | R&D shows Decode Logic from a fresh save, visibly blocked on completing Fetch Bit or Decode Bit, so the research surface is discoverable before the first task run. |
| Research requirements clarity | Tested | Research cards list unmet research, completed-work, hardware, and compute requirements as concise labeled checks before the action can be researched |
| Task/research summaries | Tested | Task cards and research compute rows show CPU cycle work once as `ops`, show a core badge only for multi-core requirements, keep cache/RAM needs and stable per-completion payouts/costs visible without repeating paid-work units as a requirement; research actions read `Research`/`Researched`, blocked action controls reserve readable reason width, and task cards do not list live state or recolor while active |
...
| Multi-core unlock | Tested | Micro Benchmark enables Multi-Core Control; buying the second core reveals repeatable Parallel Bit Count and Dual-Stream Decode jobs plus the two-core Parallelism Benchmark. The benchmark runs directly to complete the Local Scheduler gate; repeatable multi-core CPU work then requires two CPU scheduler slots and cannot dispatch directly to a core. |
| Additional cores | Tested | Cores increase parallel throughput, not single-job speed |
| Basic queue | Tested | Early queue behavior feeds operations/tasks to idle cores after queue slots are purchased; CPU and System Scheduler queues use one safe FIFO dispatch path with cache/RAM footprint admission |
...
| Four-core milestone | Tested | System Scheduler unlocks on the established two-core machine after the RAM gate; scheduler-owned RAM jobs then fund expansion to the four-core System Bus benchmark |
...
| System Scheduler unlock | Tested | Local and System Scheduler are researched unlocks; System Scheduler requires the established two-core machine, RAM Control, and at least 1 Kb RAM, then appears as a paid install outline until the first System Queue Slot is bought |
...
| System Scheduler resource priorities | Tested | Multi-stick RAM exposes Speed, Capacity, and Parallelism priorities; multi-package systems expose the same CPU priorities. Speed places fitting RAM loads on the fastest available stick while slower sticks remain usable on free channels, Capacity favors RAM headroom or CPU core/cache headroom without blocking fallback lanes, and Parallelism deliberately stripes ready RAM loads or balances CPU queues |
...
| PSU Management research | Tested | Appears after System Scheduler plus Power Telemetry and enables advanced power controls; metered billing, zero-credit shutdown, and physical PSU overload trips exist from the start |
...
| Repeatable system tasks | Tested | Memory Scrub, Queue Compaction, Power Telemetry, Bus Mirror, Shard Reconcile, and Thermal Probe remain hidden until their prerequisite research is complete, then reveal at their remaining campaign gates |
...
| PSU physical consequences | Tested | PSU draw/capacity/headroom, metered billing, zero-credit shutdown, and overload trips are active from the first screen without a research gate |
| RAM staging model | Tested | RAM extends the memory staging hierarchy after cache; RAM read/write/overwrite page jobs appear only after System Scheduler research and route through that scheduler, show loading into fixed per-stick address blocks before CPU execution, reuse released block locations after cancellation/completion, and peak per-core RAM fit gates impossible tasks while active RAM writes that exceed capacity create system-wide deadlocks that halt all active work |
...
| RAM channel research | Tested | Dual/Quad/Oct Channel RAM unlocks System Scheduler striping across up to 2/4/8 channel lanes capped by installed sticks; the scheduler's RAM priority chooses faster sticks, larger free allocations, or parallel service across ready loads. Aggregate write speed remains capped by both writer-core Hz and RAM-lane Hz, and visible bandwidth reports the current effective write rate |
...
| Power states and billing | Tested | Power states are `on`, `shuttingDown`, `off`, and `booting`; productive powered work is metered from the start while off bills zero, graceful shutdown drains current work, and idle/departure policy can power down safely |
| Zero-credit power cutoff | Tested | Billing runs from the start and clamps credits at 0; the 10-second unpaid cutoff, shutdown, and first/repeat notices are never research-gated, while a zero-credit restart receives 60 seconds of no-bill recovery grace |
| PSU reliability stress | Tested | Live PSU stress is visible from the start; overload pressure always throttles, flashes, hard-powers off, notifies, and clears active/queued work after the 10-second failure window without a research gate |
...
| CPU package tuning controls | Tested | Package frequency is purchased from one per-package Core Freq +/- control; no all-core selector or per-core frequency path exists, every core inherits its package tier and level, and Add Core lives in the Cores header instead of repeating controls in every core tile |
...
- Multi-agent review + fix pass completed July 11, 2026 (tracker:
  ...
  pass.
- Designer rulings implemented July 11-17, 2026: metered power billing,
  zero-credit shutdown, and destructive PSU overload failure are physical from
  the first tick of a fresh save; PSU Management gates advanced controls only,
  and a zero-credit restart receives 60 seconds of no-bill recovery grace. The
  flat 25%-per-socket CPU efficiency penalty is replaced
...
- Progression gating rework completed July 10, 2026: Work opens Jobs-only, with
  Automation revealed by the first buffer purchase or System Scheduler/CRON,
  Campaign revealed by System Scheduler research (legacy saves with an
  active/completed project keep Campaign; Live Operations anchors Automation),
  Contract Market revealed by CRON Scheduler research (legacy
  ...
  R&D-column purchase cards while the Automation view shows installed buffer
  purpose, capacity, limitations, and departure forecast. Rationale: contracts model unattended client
  ...
  unlocks—repeatable task Data funds research through System Scheduler,
  projects plus Live Ops fund System Bus/second CPU/CRON, and contracts fund
  Fleet expansion. Fetch Bit provides 1 Data per completion without a Data
  prerequisite, and later task Data payouts remain stable at the whole-unit
  1:10 Credit ratio on repeats.
  Scheduler Integration requires System Scheduler
...
- Active pre-live target: task work remains hardware-derived, with CPU `ops` kept separate from cache/RAM/storage/network transfer bits and payouts derived from the same exact paid work. CPU and System Scheduler queues have finite purchased slots, retain their entries through completion, and use one safe FIFO admission path with cache/RAM footprint lookahead. System child work reserves real CPU scheduler slots and waits there for idle cores and safe staging. RAM is owned by the System Scheduler: Speed places work on faster sticks first while free slower channels remain usable as fallback, Capacity favors larger free allocations without blocking fallback channels, and Parallelism deliberately stripes ready work across available researched lanes. Cache/RAM deadlocks remain active hardware failures for already-running pressure, with watchdog and cooldown recovery, while hardware installs, mixed RAM tuning, CPU-local cache/queues, and reversible upgrades follow the current rows above.
...
- Research target: new task groups and hardware categories unlock through research cards; benchmark-style compute is launched from research cards, each card lists concise research/completed-work/hardware/compute checks blocking it, and task/research rows keep needed operations/resources plus exact next-completion payouts visible even when blocked without repeating paid-work units as another requirement.

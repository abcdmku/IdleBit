...
|---|---|---|
| Primitive CPU start | Tested | New save starts with one core, 1 Hz clock, 1 b cache, 1 Hz cache load rate, 0 b RAM, hidden 1 Hz RAM load rate, credits, data, visible PSU power readouts, runnable Fetch Bit, and 2 b-gated Decode Bit visible |
...
| RAM Control gate | Tested | RAM Control appears with System Scheduler after Local Scheduler, unlocks one 256 b RAM stick at 1 Hz, and enables larger RAM/cache tasks |
...
| Second CPU unlock | Tested | Multi-core benchmark compute enables System Bus research; purchasing the matched CPU reveals locked CRON and Thermal modules with CRON at the top of the board while PSU stays visible from the start |
...
| Locked CRON/Thermal reveal | Tested | Second CPU purchase reveals locked CRON and Thermal modules while PSU remains visible from the start; CRON is placed above the system stack |
| CRON Scheduler research | Tested | Unlocks CRON controls for visible repeatable system tasks only |
| PSU Management research | Tested | Unlocks advanced PSU tuning, capacity control, and deeper efficiency readouts |
...
| Power reveal | Tested | PSU is visible from the first screen; Thermal remains locked until the matched-CPU/system stage and Thermal Control |
...
| Power states and billing | Tested | Power states are `on`, `shuttingDown`, `off`, and `booting`; off allows configuration, blocks work/CRON, and bills zero; on/transition states bill over time in cr/s with no free threshold |
| Zero-credit power cutoff | Tested | Power billing clamps credits at 0, emergency-shuts down when a bill cannot be paid, and allows a short no-bill bootstrap startup from 0 credits |
...
| Component-scoped controls | Tested | CPU, CPU-local cache, CPU-local scheduler, RAM, socket, and PSU expose relevant local actions and readouts; advanced PSU tuning and Thermal controls remain behind research gates |
...
| RAM and PSU readouts | Tested | RAM appears above the CPU, shows per-stick Stage/Load/Ready lanes in a selectable cache-style stick array with an All target, reports module frequency without summing stick speeds, and exposes new-stick plus stick capacity/frequency upgrade access; PSU readouts target draw, billing, state, capacity, stress, and efficiency from the first screen |
...
| Support module rail | Tested | Thermal renders as a locked support module at the matched-CPU stage until research unlocks controls; PSU remains an always-visible power module |
...
|---|---|
| Second CPU CRON/Thermal support | Tested |
...
- Power target: tasks do not require power directly; PSU is visible from the first screen; draw is mW/uW-scale and primarily frequency-driven; positive-credit idle time drains money; power billing clamps credits at 0 and emergency-shuts down if a bill cannot be paid; startup from 0 credits grants a short no-bill bootstrap window; PSU capacity is a reliability/stress system with throttle but no automatic task restarts; dense compute draw scales nonlinearly; `off` allows configuration while blocking work/CRON and billing zero; startup/shutdown have delays; and RAM/CPU package matching should reward efficient builds.

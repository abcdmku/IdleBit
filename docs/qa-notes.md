...
- CPU efficiency readouts appear on standalone core-array CPU views, CPU package headers, and multi-CPU array cards/tab detail views; CPU detail Cores headers show efficiency instead of an active/total core count, CPU tabs use the package letter only, and rack slot CPU bays stay compact.
- RAM stick cards list per-stick efficiency alongside size, frequency, and usage.

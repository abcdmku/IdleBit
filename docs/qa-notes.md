...
- Bootloader Research appears after System Scheduler research, unlocks for 100,000 credits, stays open as repeatable levels 1-36 starting at 10,000 credits with 1.2x cost growth to about 5.9M for level 36, and reduces boot time from 9.20s at level 1 by 0.26s per level to 0.10s.
...
- Progression gates:
  ...
  - Bootloader Research unlocks after System Scheduler research, then levels from 1 to 36 through the research card to reduce startup from 9.20s to 0.10s with 0.26s removed per level.
...
- Bootloader Research on May 25, 2026: `npx vitest run src/game/simulation.test.ts -t "Bootloader|bootloader|current saves"`, `npx vitest run src/game/simulation.test.ts`, `npm test`, `npm run typecheck`, `npm run build`, and `git diff --check` passed. Coverage verifies Bootloader Research is hidden before System Scheduler, unlocks for 100,000 credits after System Scheduler, levels through the research card from 1 to 36, starts at a 10,000 credit level cost, reaches about 5.9M credits at level 36, persists through saves, and changes startup from 9.20s at level 1 to 0.10s at level 36.

...
- Tap-and-hold task controls repeat manual dispatch for up to 30 seconds; Click Rate Tuning appears after Local Scheduler, unlocks for 500,000 credits, and levels from 10 Hz to 80 Hz without changing CRON or broad auto-repeat scope.
...
- Credits/data amounts display with RuneScape-style exact/K/M/B/T/Q/Qn/S/Sp stack counts and spaced suffixes across HUD totals, flyouts, payouts, costs, graphs, and inspect summaries.
- HUD credits/data readouts open the graph predictably: desktop clicks toggle it, mobile taps keep it open and switch to the graph/R&D view, and resource gain flyouts render above the mobile workbench chrome.
- The topbar settings icon beside data exposes toggles for hardware purchase controls and keep-screen-awake.
...
- Core grids step through logical 1x2, 2x2, 2x4, 3x4, 4x4, 3x8, 4x8, and later Nx8 layouts, then render as full-width CSS grids capped at eight columns on desktop, six on medium widths, and four on narrow widths with no horizontal scrolling; dense layouts pair cache beside the CPU-local scheduler, and compact/dense core tiles use the lower bar as the status/progress light with label stacked over frequency.
...
- CPU efficiency readouts appear on standalone core-array CPU views, CPU package headers, and multi-CPU array cards/tab detail views; CPU array cards use the same core grid/tile treatment as tab detail views; CPU detail Cores headers show efficiency instead of an active/total core count, CPU tabs use the package letter only, and rack slot CPU bays stay compact.
- RAM stick cards list per-stick efficiency alongside size, frequency, and usage, stack to a readable single column in narrow panels, and keep the All selector grouped with the RAM title instead of the New Stick control.
...
- The rack shows exactly one visible slot per owned system; completing a custom build adds one occupied slot.
- Owned systems remain one-to-one with visible rack slots, including powered-off systems.
- Premade/preconfigured system cards are deferred from the current builder UI.
- The tiered custom machine builder appears with System Catalog, does not require separate Custom Machine Assembly research, opens directly in the normal in-game System Scheduler/RAM/CPU package/cache/PSU layout, adds CPU/RAM Tier headers whose selectable values are the clock-scale Hz/kHz/MHz/GHz/THz/PHz tiers, uses normal upgrade-style +/- controls for CPU package count in the CPU header, uncapped per-CPU core count, RAM sticks, frequency, size, per-CPU scheduler slots, and PSU specs, prices PSU capacity with the same level-by-level wattage and credit curve as the normal PSU Capacity upgrade, renders each CPU as an individual core array with a Link all CPUs checkbox for shared specs, lets each CPU scheduler match that CPU's core count or use its own slot modifier, keeps shared scheduler/core/frequency controls outside CPU cards only while CPUs are linked and moves them inside each CPU card when unlinked, lets the full CPU package card select that CPU without a separate CPU tab strip, uses compact scheduler/spec controls in the builder, shows RAM channel topology in the RAM header without the runtime channel/write strip, preserves the current draft when leaving and reopening Store, switches high-core arrays to a full-width dense layout with stacked core text and no horizontal scrolling, shows RAM efficiency, and creates one complete system with the selected PSU capacity only after explicit purchase confirmation.
...
- System acquisition:
  - The builder opens directly to the normal in-game system-board layout with no premade/custom mode tabs, premade cards, or separate module picker.
  - CPU/RAM Tier controls are segmented clock-scale buttons, and the top header says System Builder with only the total price plus Review/Confirm purchase.
  - Custom builder tiers are available after System Catalog without Custom Machine Assembly research, allow inline upgrade-style configuration before purchase, preserve the current draft when the player leaves and reopens Store, validate requirements and costs, apply core/RAM/frequency/size/per-CPU scheduler modifiers to the bought system, and create exactly one system only after confirmation.
...
- Task timing:
  ...
  - Concurrent cache writes on one CPU package share the package cache load rate, so multi-core write pressure throttles like RAM writes on one serviced lane.
...
- CPU sockets present researched tier level-1 install options, list projected power increase, and keep every CPU package in a system on the same base tier.
...
- After CPU Operation Scheduler unlock, selecting All in the Cores header changes the Core Freq stepper to a package-wide control with a combined price/refund, the Core add/remove stepper stays in the Cores header, and the task route picker still offers only C/CPU/Sys routing without treating package tuning as task provisioning.
...
- Hardware layout keeps the core grid independent of the CPU package before RAM, uses compact/dense core-grid breakpoints for high-core CPU counts, stacks 9-16 core arrays into four logical columns, fills the rendered core-array width without horizontal scrolling, and moves cache beside the CPU-local scheduler once the core grid reaches the dense layout.
...
- Dual Channel RAM, Quad Channel RAM, and Oct Channel RAM research require System Scheduler and the previous channel tier where applicable; matching 2/4/8 RAM sticks are not required to complete research, but installed stick count still caps usable channel lanes.
...
- Seed schema refresh on May 24, 2026: rack-ready/trillion now build CPU packages, RAM sticks, RAM/CPU tier unlocks, PSU capacity, and empty CRON slots with current save-v6 constructors. `npx vitest run src/game/simulation.test.ts src/ui/App.failureModals.test.tsx -t "rack-ready|trillion|seed|CRON"` passed, including a `serializeSave`/`deserializeSave` round-trip assertion for `createRackReadyGameState()`.
...
- RAM stick pricing and CPU removal verification on May 25, 2026: `npx vitest run src/game/simulation.test.ts -t "CPU package|installed CPU packages|cache capacity|RAM upgrades"`, `npx vitest run src/game/simulation.test.ts`, `npx vitest run src/ui/TaskBay.queueStatus.test.tsx`, `npm test`, `npm run typecheck`, and `npm run build` passed. Browser smoke at `http://localhost:6173/` loaded the IdleBit app shell with no console errors. Coverage verifies RAM stick installs price at 1x/2x/4x the base tier cost and stale second-CPU flags cannot keep a removed CPU package visible.
- CPU cache write sharing on May 24, 2026: `npx vitest run src/game/simulation.test.ts`, `npm test`, and `npm run typecheck` passed. Coverage verifies four active cores on one CPU package share that package's cache write lane instead of each receiving the full cache speed independently.
...
- System builder system-view pass on May 27, 2026: `npm test -- src/ui/HardwareBoard.rack.test.tsx`, `npm test`, `npm run typecheck`, `npm run build`, and `git diff --check` passed. Browser smoke at `http://127.0.0.1:6178/` verified the custom builder renders in the normal in-game System Scheduler/RAM/CPU package/cache/PSU layout, with CPU and RAM clock-tier buttons, no premade/custom mode tabs, no premade cards, no separate module picker, no preview progress bars or custom power meter, top System Builder price/review controls, CPU package +/- and Link all CPUs in the CPU header, individual CPU core arrays with shared Core/Freq/Scheduler controls outside CPU cards only while linked and embedded per-card controls while unlinked, per-CPU scheduler slot modifiers that can match that CPU's cores, full-card CPU package selection, RAM channel header labels without the runtime channel/write strip, Store draft preservation across navigation, RAM Stick +/- controls in the section headers, full-width dense core arrays above eight cores with stacked core text and no horizontal scrolling, RAM efficiency readouts, working size/frequency modifier steppers that update the total price, projected PSU draw/cost-to-run status from core/RAM/cache/frequency changes, and no console errors.
- Offline hardware editing and store PSU purchase fix on May 28, 2026: `npm test -- --run src/game/simulation.test.ts src/ui/HardwareBoard.power.test.tsx src/ui/HardwareBoard.systemManagement.test.tsx src/ui/HardwareBoard.rack.test.tsx src/ui/uiActions.test.ts`, `npm test`, `npm run typecheck`, `npm run build`, and `git diff --check` passed. Coverage verifies custom store PSU payloads cannot downgrade the selected PSU SKU below its base level and that the UI adapter maps power-group PSU selections and levels into game purchases; browser smoke at `http://127.0.0.1:6180/?seed=trillion` verified an off hardware board has `power-offline` without `power-transitioning`, Boot is enabled, and PSU/RAM upgrade buttons remain enabled with `pointer-events: auto`.
